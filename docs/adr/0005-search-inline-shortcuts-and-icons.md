...
Shortcut settings use click-to-record for one key chord. Modifier-only presses are ignored, Escape cancels, and Backspace clears. A conflicting chord is rejected and the existing binding remains unchanged. The settings schema is strict: an existing file that cannot parse the current fields is a startup error; no default fallback, migration, or compatibility field is added.
